...
const { getDriver, listTools, listKinds } = require("../connectors");

const GOOGLE_KINDS = new Set(["gmail", "google_calendar"]);
...
router.get("/oauth/google/callback", async (req, res) => {
  try {
    ...
    if (!GOOGLE_KINDS.has(connector.kind)) return res.status(400).send("unsupported kind");

    const driver = getDriver(connector.kind);
    if (!driver) return res.status(400).send("no driver");
    const tok = await driver.exchangeCode(code);
    const email = await driver.fetchUserEmail(tok.access_token);
    driver.persistTokens(connector, tok, email);
...
router.get("/:id/oauth/start", auth, async (req, res) => {
  try {
    ...
    if (!GOOGLE_KINDS.has(connector.kind)) return res.status(400).send({ ok: false, code: "UNSUPPORTED_KIND" });

    const driver = getDriver(connector.kind);
    if (!driver?.buildAuthUrl) return res.status(400).send({ ok: false, code: "NO_DRIVER" });
    const state = jwt.sign({ connector_id: connector._id.toString(), user_id: req.user._id.toString() }, JWT_SECRET, { expiresIn: "10m" });
    const url = driver.buildAuthUrl(state);
...
router.get("/drivers", auth, async (req, res) => {
  return res.status(200).send({ ok: true, data: { kinds: listKinds() } });
});
